...
void UpMushroom::CollectItem(Mario& mario)
{
	//mario.ChangeToSuper();
	active = false;
}

void UpMushroom::Init(float x, float y)
{
...
void UpMushroom::RenderGlobal(Camera& camera)
{
	camera.GlobalRenderImage(image);
}

//tmp
void UpMushroom::OnHitSide(RigidBody& player)
{
	SpawnItem();
}

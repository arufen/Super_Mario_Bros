...
//„Ç´„Çπ„Çø„É†„Éè„É≥„Éâ„É´„ÇíÊåÅ„Å§„ÄÅÁâπÂà•„Å™Ê©üËÉΩ„ÅÆ„Å™„ÅÑÂõ∫ÂÆö„Éñ„É≠„ÉÉ„ÇØ„ÇíÁîüÊàê„Åô„Çã
...
//Stair looking ground (ÈöéÊÆµ„Çí‰Ωú„ÇãÈñ¢Êï∞Ôºâ
...
vector<Collidable*> StageManager::GetCollidables()
{
	...
	//ÉSÅ[ÉãÉ|Å[ÉãÇï®óùîªíËÉäÉXÉgÇ…í«â¡
	for (auto* pole : goalPoles)
	{
		result.push_back(pole); // á@ Ç∑ÇËî≤ÇØÇÈÉ|Å[Éãñ{ëÃÇÃîªíË
		result.push_back(&(pole->baseCollider)); // áA å≈Ç¢ìyë‰ÇÃîªíË
	}
...
void StageManager::Init(Stage stageNumber)
{
	...
	{
		//Stage (ground1)
		CreateGrounds(0, 13, 68, 14); //from (0, 13) to (20, 14) | (0, 13)„Åã„Çâ(68, 14)„Åæ„Åß
		...
		// ÉSÅ[ÉãÉ|Å[ÉãÇ X=12700 ÇÃà íuÇ…ê›íu
		// Å¶ Yç¿ïW(192.0f)ÇÕâºÇ≈Ç∑ÅBgoal_pole.pngÇÃâÊëúÇÃçÇÇ≥Ç…çáÇÌÇπÇƒÅA
		// è∞(Y=832)Ç…Ç“Ç¡ÇΩÇËÇ≠Ç¡Ç¬Ç≠ÇÊÇ§Ç…î˜í≤êÆÇµÇƒÇ≠ÇæÇ≥Ç¢ÅB
	...
	{
		//Reset mario („Éû„É™„Ç™„Çí„É™„Çª„ÉÉ„ÉàÔºâ
		MainMario.position.Set(2 * BLOCK_SIZE, 5 * BLOCK_SIZE);

		//Reset Camera („Ç´„É°„É©„Çí„É™„Çª„ÉÉ„ÉàÔºâ
		...
		//Reset Stqage („Çπ„ÉÜ„Éº„Ç∏„Çí„É™„Çª„ÉÉ„ÉàÔºâ
